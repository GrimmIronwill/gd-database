...
func _value_field() -> DBFieldDef:
	...
	if _field_def:
		...
		fd.dict_key_enum_ref = _field_def.dict_key_enum_ref
		fd.ref_table_name = _field_def.ref_table_name
...
func _rebuild_key_editor() -> void:
	...
	match _key_type():
		...
		DBFieldDef.FieldType.BOOL:
			_k_check = CheckBox.new()
			_k_check.text = "true / false"
		...
		DBFieldDef.FieldType.TABLE_REF:
			_k_line = LineEdit.new()
			_k_line.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			_k_line.placeholder_text = "entry_id…"
			_k_line.text_changed.connect(func(t): _current_key = t)
			_key_host.add_child(_k_line)

			var pick := Button.new()
			pick.text = "Pick…"
			pick.tooltip_text = "Choose row ID for dictionary key"
			pick.pressed.connect(_on_pick_key_table_ref)
			_key_host.add_child(pick)
...
func _write_key_editor(v: Variant) -> void:
	...
	match _key_type():
		DBFieldDef.FieldType.INT, DBFieldDef.FieldType.FLOAT:
			if _k_spin:
				_k_spin.set_value_no_signal(float(_current_key))

		DBFieldDef.FieldType.BOOL:
			if _k_check:
				_k_check.set_pressed_no_signal(bool(_current_key))

		DBFieldDef.FieldType.ENUM:
			if _k_enum and _k_enum.item_count > 0:
				_k_enum.selected = clampi(
					int(_current_key),
					0,
					_k_enum.item_count - 1
				)
...
func _rebuild_value_editor() -> void:
	...
	match _value_type():
		...
		DBFieldDef.FieldType.BOOL:
			_v_check = CheckBox.new()
			_v_check.text = "true / false"
		...
		DBFieldDef.FieldType.TABLE_REF:
			_v_line = LineEdit.new()
			_v_line.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			_v_line.placeholder_text = "entry_id…"
			_v_line.text_changed.connect(func(t): _current_value = t)
			_value_host.add_child(_v_line)

			var pick := Button.new()
			pick.text = "Pick…"
			pick.tooltip_text = "Choose row ID for dictionary value"
			pick.pressed.connect(_on_pick_value_table_ref)
			_value_host.add_child(pick)
		...
		DBFieldDef.FieldType.VECTOR2:
			...
			_v_vx = _make_spin(0.001, true)
			_v_vx.value_changed.connect(func(_v): _update_vector())
			_value_host.add_child(_v_vx)

			_value_host.add_child(_axis_label("Y"))
			_v_vy = _make_spin(0.001, true)
			_v_vy.value_changed.connect(func(_v): _update_vector())
		...
		DBFieldDef.FieldType.VECTOR3:
			...
			_v_vz = _make_spin(0.001, true)
			_v_vz.value_changed.connect(func(_v): _update_vector())
...
func _write_value_editor(v: Variant) -> void:
	...
	match _value_type():
		DBFieldDef.FieldType.INT, DBFieldDef.FieldType.FLOAT:
			if _v_spin:
				_v_spin.set_value_no_signal(float(_current_value))

		DBFieldDef.FieldType.BOOL:
			if _v_check:
				_v_check.set_pressed_no_signal(bool(_current_value))

		DBFieldDef.FieldType.STRING, \
		DBFieldDef.FieldType.RESOURCE_REF, \
		DBFieldDef.FieldType.TABLE_REF:
			if _v_line:
				_v_line.text = str(_current_value)

		DBFieldDef.FieldType.ENUM:
			if _v_enum and _v_enum.item_count > 0:
				_v_enum.selected = clampi(
					int(_current_value),
					0,
					_v_enum.item_count - 1
				)

		DBFieldDef.FieldType.VECTOR2:
			var v2: Vector2 = (
				_current_value
				if _current_value is Vector2
				else Vector2.ZERO
			)

			if _v_vx:
				_v_vx.set_value_no_signal(v2.x)
			if _v_vy:
				_v_vy.set_value_no_signal(v2.y)

		DBFieldDef.FieldType.VECTOR3:
			var v3: Vector3 = (
				_current_value
				if _current_value is Vector3
				else Vector3.ZERO
			)

			if _v_vx:
				_v_vx.set_value_no_signal(v3.x)
			if _v_vy:
				_v_vy.set_value_no_signal(v3.y)
			if _v_vz:
				_v_vz.set_value_no_signal(v3.z)

		DBFieldDef.FieldType.COLOR:
			_val_color = (
				_current_value
				if _current_value is Color
				else Color.WHITE
			)
			_style_color_button(_val_color)
		...
		DBFieldDef.FieldType.NESTED_OBJECT:
			if _v_complex_btn:
				var dd: Dictionary = (
					_current_value
						if _current_value is Dictionary
						else {}
				)
...
func _on_pick_key_table_ref() -> void:
	_close_child_dialog()

	if _database == null:
		push_warning("[Dictionary Editor] База данных не задана для Table Ref ключа.")
		return

	if _field_def == null or _field_def.ref_table_name.is_empty():
		push_warning("[Dictionary Editor] Не задана целевая таблица для Table Ref ключа.")
		return
	...
	picker.ref_confirmed.connect(func(entry_id: String) -> void:
		_current_key = entry_id

		if _k_line:
			_k_line.text = entry_id
	...
	picker.open(
		_database,
		_field_def.ref_table_name,
		str(_current_key)
	)

func _on_pick_value_table_ref() -> void:
	_close_child_dialog()

	if _database == null:
		push_warning("[Dictionary Editor] База данных не задана для Table Ref значения.")
		return

	if _field_def == null or _field_def.ref_table_name.is_empty():
		push_warning("[Dictionary Editor] Не задана целевая таблица для Table Ref значения.")
		return
	...
	picker.ref_confirmed.connect(func(entry_id: String) -> void:
		_current_value = entry_id

		if _v_line:
			_v_line.text = entry_id
...
func _on_edit_array_value() -> void:
	...
	if _field_def:
		...
		fd.array_element_type = DBFieldDef.FieldType.STRING
	...
	ed.confirmed_array.connect(func(result: Array) -> void:
		...
		if _v_complex_btn:
			_v_complex_btn.text = "[Array: %d]" % result.size()
